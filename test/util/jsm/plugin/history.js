...
test('history', t => {
  ...
  fsm.reset();           t.is(fsm.state, 'solid');  t.deepEqual(fsm.history, ['solid' ]);
  fsm.melt();            t.is(fsm.state, 'liquid'); t.deepEqual(fsm.history, [ 'solid', 'liquid' ])
  fsm.state='solid';     t.is(fsm.state, 'solid');  t.deepEqual(fsm.history, ['solid'])
...
test('history with configurable names', t => {
  ...
  var fsm = new StateMachine({
    ...
    plugins: [
      new StateMachine.PLUGIN.HISTORY({ name: 'memory', future: 'yonder' })
    ]
  })
...
test('history can be limited to N entries', t => {
  var fsm = new StateMachine({
    ...
    plugins: [
      new StateMachine.PLUGIN.HISTORY({ max: 3 })
    ]
  })
